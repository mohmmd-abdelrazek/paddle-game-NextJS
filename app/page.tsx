...
export default function Game() {
  ...
  const move = useCallback(() => {
    ...
    if (isLeftArrowPressed.current && boardLeft > 0) {
      setBoardX((bx) => bx - boardSpeed);
    }
    if (
      isRightArrowPressed.current &&
      boardLeft < containerWidth - boardWidth
    ) {
      setBoardX((bx) => bx + boardSpeed);
    }
  ...
  return (
    <div
      id="container"
      className="relative h-full w-full bg-[#f0e39d] md:w-4/5"
    >
      <Header
        score={score}
        level={level}
